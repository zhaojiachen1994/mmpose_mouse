...
class MouseNet_3d(BasePose):
    ...
    def forward(self,
        ...
        if return_loss:
            return self.forward_train(img,
                                      ...
                                      proj_matrices,
                                      target,
                                      target_weight.reshape(-1, *target_weight.shape[2:]),
                                      **kwargs
                                      )
    ...
    def forward_train(self, img, img_metas, proj_matrices, target,
        ...
        # if return loss
        losses = dict()
        if self.with_keypoint_head and target is not None and self.train_cfg.get('supervised_2d', True):
    ...
    def foraward_dummy(self, img):
        ...
        hidden_features = self.backbone(img)
        if self.with_neck:
            hidden_features = self.neck(hidden_features)
        if self.with_keypoint:
            heatmap = self.keypoint_head(hidden_features)
        if self.with_feature:
